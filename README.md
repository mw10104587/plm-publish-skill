...
> 在 Claude Code 打 `/plm-publish` 就能用。
...
> `plugins/plm-publish/skills/plm-publish` 這個資料夾,
> 下載到我的 `~/.claude/skills/plm-publish`。
...
在 Claude Code 裡輸入 `/plm-publish`,後面接你要做的事:

| 你想做的事 | 這樣打 |
| --- | --- |
| 上稿某個 Notion 頁面 | `/plm-publish <貼上 Notion 連結>` |
| 開新一期的草稿結構 | `/plm-publish 期刊範本` |
| 新增一篇典藏文章草稿 | `/plm-publish 文章範本` |
| 不確定 / 想被引導 | 直接打 `/plm-publish`,它會問你 |
...
- 用**方式 B** 裝的:再跟 Claude 說一次「請更新 plm-publish 技能」即可。

---

## 遇到問題?

- 顯示「還沒看到 plm-publish 的安裝」→ 安裝沒成功,重做上面的安裝步驟。
